...
public class FileController {
    ...
    @PostMapping("/uploadKeJian")
    public Result uploadKeJian(@RequestParam MultipartFile file) throws IOException {
        String originalFilename = file.getOriginalFilename();
        String type = FileUtil.extName(originalFilename);
        long size = file.getSize();
    ...
    public Result uploadVideo(@RequestParam MultipartFile file) throws IOException {
        ...
        if(!type.equals("avi") && !type.equals("mp4")){
            return Result.error("400","上传的文件必须是mp4或者是avi");
        }
        long size = file.getSize();
        ...
        result.put("fileurl",saveFile.getUrl());
        result.put("filename",saveFile.getName());
        result.put("filesize",saveFile.getSize());
        result.put("md5",saveFile.getMd5());
        return Result.success(result);
    }
...
//        Map<String,Object> map = new HashMap<>();
    ...
    /**
     * 文件下载接口   http://localhost:8999/file/{fileUUID}
     * @param fileUUID
     * @param response
     * @throws IOException
     */
